...
(rum/defc cabecalho []
  [:header
   [:img#logo-semente {:src "/res/img/logo-nacional.svg"}]
      ...
      [:button#botom-procura {:type :submit}
       [:svg#icono-procura {:viewBox "0 0 40 40"
        ...
        [:path {:fill style/azul-semente
                :fill-rule :evenodd
                :d search-icon-path}]]]]]
    (icono-social "#twitter" twitter-path)
    (icono-social "#facebook" facebook-path)
    (icono-social "#youtube" youtube-path)]])
...
(rum/defc navegacom [escolhido]
  [:nav
   [:div#fondo-navegacom
    ...
    (item-nav escolhido "/nacional/principios" "princípios")
    (item-nav escolhido "/nacional/recursos" "recursos")
    (item-nav escolhido "/nacional/contato" "contato")
    (item-nav escolhido "/nacional/associa-te" "associa-te")]])

(rum/defc pe []
  [:footer {:style {:clear :both}}
   [:div.barra
    [:img {:src "/res/img/ramalho.svg"}]]
   [:div#contedor-texto-pe
    [:div "© 2018 Escola de Ensino Galego Semente"]]])
